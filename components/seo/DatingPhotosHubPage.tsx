    ...
    <div className="min-h-screen bg-[#f7f5f3] text-zinc-950">
      ...
      <main className="overflow-hidden pt-24">
        ...
        <section className="mx-auto max-w-7xl px-5 py-20">
          <p className="text-xs font-black uppercase tracking-[.2em] text-[#ff6f00]">Explore the topic cluster</p>
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-zinc-600">Use the platform pages for product-specific solutions, the guides for selecting your final profile photos, and the examples to understand how one generated idea changes across four photographic roles. Example concepts are illustrative, not selectable presets.</p>
          <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{datingHubSpokes.map((spoke) => { const shoot = getDatingShoot(spoke.shoot); return <Link key={spoke.href} href={spoke.href} className="group overflow-hidden rounded-3xl border border-zinc-200 bg-white"><div className="relative aspect-[16/10] bg-zinc-100">{shoot && <Image src={shoot.frames[0].src} alt={shoot.frames[0].alt} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition duration-300 group-hover:scale-[1.02]" />}</div><div className="p-6"><h3 className="text-xl font-black">{spoke.title}</h3><p className="mt-3 leading-7 text-zinc-600">{spoke.description}</p><span className="mt-5 inline-flex items-center gap-1 font-bold text-[#ff6f00]">Open this page <ArrowRight className="h-4 w-4" /></span></div></Link>})}</div>
