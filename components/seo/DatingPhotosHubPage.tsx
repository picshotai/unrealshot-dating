    ...
    <div className="min-h-screen bg-[#f7f5f3] text-zinc-950">
      ...
      <main className="overflow-hidden pt-24">
        <section className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-16 lg:grid-cols-[1.04fr_.96fr] lg:py-24">
          <div>
            ...
            <div className="mt-9 flex flex-wrap items-center gap-4">
              <Link href="/login" className={ctaClass}>Create my dating photos <ArrowRight className="h-4 w-4" /></Link>
        ...
        <section className="mx-auto max-w-6xl px-5 py-20">
          <div className="grid gap-10 rounded-[2rem] bg-zinc-950 p-8 text-white lg:grid-cols-[1fr_.8fr] lg:p-12"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#ff6f00]">From selfies to a complete set</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">15 shoots delivered within 30 minutes</h2><p className="mt-5 text-lg leading-8 text-zinc-300">Upload 4–6 recent solo selfies and answer three practical questions about your preferred look and genuine interests. UnrealShot uses those references to guide likeness; it does not train a custom model.</p><p className="mt-5 text-lg leading-8 text-zinc-300">The $39 one-time package contains 60 photos and 15 individual Photo Retakes. Use retakes when a useful scene contains one weak likeness, expression or composition.</p><div className="mt-8 flex flex-wrap gap-4"><Link href="/login" className={ctaClass}>Create my 15 shoots</Link><Link href="/how-it-works" className="rounded-xl border border-zinc-700 px-6 py-3.5 font-bold">See the full process</Link></div></div><dl className="grid grid-cols-2 overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900">{[["References", "4–6 selfies"], ["Scenes", "15 shoots"], ["Output", "60 photos"], ["Retakes", "15 frames"], ["Delivery", "Within 30 min"], ["Price", "$39 once"]].map(([label, value]) => <div key={label} className="border-b border-r border-zinc-800 p-5"><dt className="text-xs uppercase tracking-wide text-zinc-500">{label}</dt><dd className="mt-2 font-black">{value}</dd></div>)}</dl></div>
        </section>
        ...
        <section className="mx-auto max-w-5xl px-5 py-20 text-center"><div className="rounded-[2rem] bg-[#ff6f00] px-7 py-14 text-white"><p className="text-xs font-black uppercase tracking-[.2em] text-orange-100">Your camera roll does not need another random image</p><h2 className="mt-3 text-3xl font-black sm:text-5xl">Build the complete dating photo lineup</h2><p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-orange-50">15 coherent shoots · 60 photos · 15 individual Photo Retakes · delivered within 30 minutes · $39 once.</p><Link href="/login" className="mt-8 inline-flex items-center gap-2 rounded-xl bg-zinc-950 px-7 py-4 font-black">Generate my dating photos <ArrowRight className="h-4 w-4" /></Link></div></section>
      </main>
      <Footer />
