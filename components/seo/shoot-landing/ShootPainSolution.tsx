...
export default function ShootPainSolution({ shoot, copy }: { shoot: DatingShoot; copy: ShootLandingCopy }) {
  return (
    <section className="bg-white py-20 sm:py-24">
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="mt-6 grid gap-6 rounded-3xl bg-zinc-950 p-7 text-white shadow-2xl md:grid-cols-[.72fr_1.28fr] md:p-10"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#ff6f00]">The UnrealShot solution</p><h3 className="mt-3 text-3xl font-black">One believable {shoot.name.toLowerCase()} moment, not four unrelated generations</h3></div><div className="grid gap-4 sm:grid-cols-2">{["Your reference selfies guide current appearance", `The ${shoot.location.toLowerCase()} stays consistent`, `The ${shoot.clothing.toLowerCase()} stays consistent`, "Four camera roles give you useful choices"].map((item) => <div key={item} className="flex gap-3 rounded-2xl border border-zinc-800 bg-zinc-900 p-4 text-sm leading-6 text-zinc-200"><Check className="mt-1 h-4 w-4 shrink-0 text-[#ff6f00]" />{item}</div>)}</div></div>
      </div>
    </section>
  )
}
