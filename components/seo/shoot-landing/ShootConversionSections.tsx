  ...
  return (
    <>
      <section className="bg-[#111111] py-20 text-white sm:py-24">
        <div className="mx-auto max-w-7xl px-5">
          <div className="grid gap-8 rounded-3xl border-2 border-dashed border-zinc-800 bg-[#161616] p-3 lg:grid-cols-2">
            ...
            <div className="flex flex-col justify-between rounded-2xl border border-dashed border-zinc-800 bg-black p-7 sm:p-10"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#ff6f00]">{ui.detailsEyebrow}</p><h3 className="mt-3 text-3xl font-black">{ui.detailsHeading}</h3><p className="mt-5 text-lg leading-8 text-zinc-300">{copy.selectionNote}</p><div className="mt-7 space-y-3">{ui.detailsBullets.map((item) => <div key={item} className="flex gap-3 text-sm text-zinc-300"><Check className="mt-0.5 h-4 w-4 shrink-0 text-[#ff6f00]" />{item}</div>)}</div></div><PublicLink href="/dating-photos/activity" className="mt-8 inline-flex items-center gap-2 font-bold text-[#ff6f00]">{ui.detailsLink} <ArrowRight className="h-4 w-4" /></PublicLink></div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#111111] px-5 py-20 text-white sm:py-24">
        <div className="mx-auto max-w-6xl rounded-3xl border-2 border-dashed border-zinc-800 bg-[#161616] p-3 shadow-2xl"><div className="grid gap-3 lg:grid-cols-[1.2fr_.8fr]"><div className="rounded-2xl border border-dashed border-zinc-700/70 bg-[#1c1c1c] p-7 sm:p-10"><span className="rounded-full bg-[#ff6f00]/15 px-3 py-1 text-xs font-black uppercase tracking-wide text-[#ff6f00]">{ui.packageBadge}</span><h2 className="mt-5 text-3xl font-black tracking-tight sm:text-5xl">{ui.packageHeading}</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-zinc-300">{ui.packageDescription}</p><div className="mt-8 grid gap-4 sm:grid-cols-2">{ui.packageBullets.map((item) => <div key={item} className="flex gap-3 text-sm font-semibold text-zinc-200"><Check className="h-4 w-4 shrink-0 text-[#ff6f00]" />{item}</div>)}</div></div><div className="flex flex-col justify-between rounded-2xl border border-dashed border-zinc-800 bg-black p-7 text-center sm:p-10"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#ff6f00]">{ui.paymentEyebrow}</p><p className="mt-4 text-6xl font-black tracking-tighter">{ui.paymentPrice}</p><p className="mt-2 text-zinc-400">{ui.paymentSubtext}</p><div className={`mx-auto my-7 grid max-w-sm gap-2 ${copy.gallery.length > 2 ? "grid-cols-3" : "grid-cols-2"}`}>{copy.gallery.slice(0, 3).map((image) => <div key={image.src} className="relative aspect-[3/4] overflow-hidden rounded-lg border border-zinc-700"><Image src={image.src} alt="" fill sizes="100px" className="object-cover" /></div>)}</div></div><div><Link href="/login" className="flex w-full items-center justify-center gap-2 rounded-md bg-[#ff6f00] px-6 py-4 font-bold text-white transition hover:bg-[#e96500]">{ui.startCta} <ArrowRight className="h-4 w-4" /></Link><p className="mt-5 flex items-center justify-center gap-2 text-xs text-zinc-500"><ShieldCheck className="h-4 w-4 text-[#ff6f00]" />{ui.secureText}</p></div></div></div></div>
      </section>

      <section className="bg-[#f7f5f3] py-20 sm:py-24">
        <div className="mx-auto max-w-7xl px-5"><div className="mx-auto max-w-4xl text-center"><p className="text-xs font-black uppercase tracking-[.2em] text-[#ff6f00]">{ui.moreEyebrow}</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">{ui.moreHeading}</h2><p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-zinc-600">{ui.moreDescription}</p></div><div className="mt-12 grid gap-6 md:grid-cols-3">{related.map((item) => { const relatedCopy = getLocalizedShootLandingContent(item.slug, locale); const relatedPage = getLocalizedShootPage(item.slug, locale); if (!relatedCopy || !relatedPage) return null; const cover = relatedCopy.gallery[1] ?? relatedCopy.gallery[0]; if (!cover) return null; return <PublicLink key={item.slug} href={`/dating-photos/shoots/${item.slug}`} className="group flex flex-col overflow-hidden rounded-3xl border border-zinc-200 bg-white transition duration-300 hover:shadow-lg"><div className="relative aspect-[4/5] bg-zinc-100"><Image src={cover.src} alt={cover.alt} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition duration-300 group-hover:scale-[1.02]" /></div><div className="flex flex-1 flex-col justify-between p-6"><div><p className="text-xs font-black uppercase tracking-wide text-[#ff6f00]">{ui.relatedLabel}</p><h3 className="mt-2 text-2xl font-black">{relatedPage.shoot.name} · {ui.examplesEyebrow}</h3><p className="mt-3 line-clamp-3 leading-7 text-zinc-600">{relatedCopy.heroDescription}</p></div><span className="mt-5 inline-flex items-center gap-2 font-bold text-[#ff6f00]">{ui.exploreRelated} <ArrowRight className="h-4 w-4" /></span></div></PublicLink> })}</div><div className="mt-8 text-center"><PublicLink href="/dating-photos/examples" className="font-bold text-[#ff6f00] underline decoration-2 underline-offset-4">{ui.moreLink}</PublicLink></div></div>
      </section>
