      ...
      <main className="overflow-hidden pt-24">
        <section className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-16 lg:grid-cols-[1.02fr_.98fr] lg:py-24">
          <div>
            ...
            <ul className="mt-7 grid gap-3 text-sm font-semibold text-zinc-700 sm:grid-cols-2">
              {content.heroBullets.map((item) => <li key={item} className="flex gap-2"><span aria-hidden="true" className="text-[#e85d2a]">✓</span>{item}</li>)}
            </ul>
            <div className="mt-9 flex flex-wrap items-center gap-4">
              <Link href="/dashboard" className={ctaClass}>Create my {content.app} photos</Link>
              <Link href="/dating-photos/examples" className="font-bold text-zinc-700 underline decoration-zinc-300 underline-offset-4">See complete shoots</Link>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 rounded-[2rem] bg-zinc-950 p-3 shadow-2xl">
            {examples.slice(0, 4).map((shoot, index) => shoot && (
              <figure key={shoot.slug} className={`relative overflow-hidden rounded-2xl bg-zinc-800 ${index % 2 ? "translate-y-5" : "-translate-y-1"}`}>
                ...
                <figcaption className="absolute inset-x-2 bottom-2 rounded-lg bg-black/70 px-2 py-1.5 text-[10px] font-bold uppercase tracking-wide text-white backdrop-blur">AI-generated · {shoot.name}</figcaption>
              </figure>
            ))}
          </div>
        </section>
        ...
        <section className="border-y border-zinc-200 bg-white py-20">
          <div className="mx-auto max-w-7xl px-5">
            <p className="text-xs font-black uppercase tracking-[.2em] text-[#d45125]">The profile problem</p>
            ...
            <div className="mt-12 grid gap-5 md:grid-cols-3">{content.problems.map((problem, index) => <article key={problem.title} className="rounded-3xl border border-zinc-200 bg-[#f7f5f3] p-7"><span className="text-sm font-black text-[#d45125]">0{index + 1}</span><h3 className="mt-3 text-xl font-black">{problem.title}</h3><p className="mt-3 leading-7 text-zinc-600">{problem.body}</p></article>)}</div>
          </div>
        </section>

        <section className="mx-auto max-w-7xl px-5 py-20">
          <p className="text-xs font-black uppercase tracking-[.2em] text-[#d45125]">The UnrealShot solution</p>
          <h2 className="mt-3 max-w-4xl text-3xl font-black tracking-tight sm:text-5xl">Fifteen complete shoots. Four useful frames in every scene.</h2>
        ...
        <section className="bg-[#1b1816] py-20 text-white">
          <div className="mx-auto max-w-6xl px-5">
            <p className="text-xs font-black uppercase tracking-[.2em] text-orange-400">A working lineup</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">Six different jobs—not six versions of one selfie</h2>
            <div className="mt-12 divide-y divide-zinc-800 border-y border-zinc-800">{content.lineup.map((item) => <article key={item.slot} className="grid gap-3 py-7 sm:grid-cols-[110px_220px_1fr]"><span className="font-mono text-sm text-orange-400">{item.slot}</span><h3 className="text-lg font-bold">{item.role}</h3><p className="leading-7 text-zinc-300">{item.explanation}{item.shootSlug && <> <Link className="font-semibold text-orange-300 underline underline-offset-4" href={`/dating-photos/shoots/${item.shootSlug}`}>See this shoot type.</Link></>}</p></article>)}</div>
          </div>
        </section>

        <section className="mx-auto max-w-4xl space-y-16 px-5 py-20">
          {content.sections.map((section) => <section key={section.heading}><h2 className="text-3xl font-black tracking-tight sm:text-4xl">{section.heading}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph} className="mt-5 text-lg leading-8 text-zinc-600">{paragraph}</p>)}{section.bullets && <ul className="mt-6 grid gap-3 rounded-3xl border border-zinc-200 bg-white p-7 sm:grid-cols-2">{section.bullets.map((item) => <li key={item} className="flex gap-2 leading-7 text-zinc-700"><span className="text-[#d45125]">✓</span>{item}</li>)}</ul>}</section>)}
        </section>

        <section className="border-y border-zinc-200 bg-white py-20">
          <div className="mx-auto max-w-7xl px-5"><h2 className="text-3xl font-black tracking-tight sm:text-5xl">Four product examples, shown as complete scenes</h2><p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">Every image below is an AI-generated UnrealShot product example. Open a shoot to see all four roles, individual captions and where that scene belongs in a profile.</p><div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{examples.map((shoot) => shoot && <Link href={`/dating-photos/shoots/${shoot.slug}`} key={shoot.slug} className="group overflow-hidden rounded-3xl border border-zinc-200 bg-[#f7f5f3]"><div className="relative aspect-[4/5]"><Image src={shoot.frames[0].src} alt={shoot.frames[0].alt} fill sizes="(max-width: 640px) 100vw, 25vw" className="object-cover transition duration-300 group-hover:scale-[1.02]" /></div><div className="p-5"><p className="text-xs font-bold uppercase text-[#d45125]">AI-generated · 4 frames</p><h3 className="mt-2 text-xl font-black">{shoot.name}</h3><span className="mt-3 inline-block font-semibold">Open complete shoot →</span></div></Link>)}</div></div>
        </section>
        ...
        <section className="mx-auto max-w-5xl px-5 py-20 text-center"><div className="rounded-[2rem] bg-zinc-950 px-6 py-14 text-white sm:px-12"><p className="text-xs font-black uppercase tracking-[.2em] text-orange-400">$39 once · no subscription</p><h2 className="mt-3 text-3xl font-black sm:text-5xl">Build all 15 shoots in one delivery</h2><p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-zinc-300">Upload 4–6 selfies and receive 60 photos within 30 minutes, with 15 individual Photo Retakes included.</p><Link href="/dashboard" className={`${ctaClass} mt-8`}>Create my {content.app} photos</Link></div></section>
      </main>
      <Footer />
