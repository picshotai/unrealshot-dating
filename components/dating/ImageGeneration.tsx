...
// Ported to framer-motion (this repo has no `motion` package) and used for the
// reshoot flow in the dating studio.
...
function DitherField({
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        className="absolute inset-0 size-full text-zinc-400"
      />
    </motion.div>
  );
}
    ...
    >
      ...
      >
        <div
          ...
          style={{ aspectRatio }}
          className="relative isolate w-full overflow-hidden rounded-xl bg-zinc-900"
        >
          <motion.div
          ...
          <div className="mt-3 text-left">
            {showStatus ? (
              <div
                aria-live="polite"
                className={cn(
                  "flex min-h-5 items-center gap-2 text-sm font-medium text-zinc-200",
        ...
        {status === "error" && onRetry ? (
          <motion.button
            ...
            className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-full px-3 text-sm font-medium text-zinc-200 outline-none transition-colors hover:bg-zinc-800 focus-visible:ring-2 focus-visible:ring-zinc-600"
          >
