  ...
  return (
    <div className="min-h-screen bg-[#F7F5F3] font-[family-name:var(--font-inter)] text-gray-900">
      ...
      <main className="pt-20 md:pt-24 pb-20">
        ...
        <section className="max-w-6xl mx-auto px-4 pb-20">
          <div className="bg-white rounded-3xl border border-gray-200/80 shadow-[0_12px_50px_-15px_rgba(0,0,0,0.08)] overflow-hidden grid lg:grid-cols-12">
            ...
            <div className="lg:col-span-5 bg-gradient-to-br from-[#111111] to-[#181818] p-8 sm:p-12 flex flex-col justify-between text-white relative">
              ...
              <div><Link href="/login" className="block w-full"><Button className="w-full group relative bg-[#ff6f00] hover:bg-[#ff6f00]/90 text-white rounded-xl overflow-hidden cursor-pointer py-6 font-bold text-base shadow-xl shadow-orange-500/20 active:scale-[0.99] transition-all">{t("package.cta")}</Button></Link><div className="mt-6 flex flex-col items-center gap-2 text-center text-xs text-gray-400"><div className="flex items-center justify-center gap-2 text-gray-300"><Lock className="w-3.5 h-3.5 text-green-400" /><span>{t("package.secure")}</span></div><p className="text-[11px] text-gray-500">{t("package.guarantee")}</p></div></div>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-5xl mx-auto px-4"><div className="bg-[#111111] rounded-3xl p-8 sm:p-12 text-white text-center border-2 border-dashed border-zinc-800 shadow-2xl"><h2 className="text-3xl sm:text-4xl font-bold mb-3 font-[var(--font-inter-tight)]">{t("cta.heading")}</h2><p className="text-gray-300 text-sm sm:text-base max-w-xl mx-auto mb-8 leading-relaxed">{t("cta.description")}</p><Link href="/login"><Button className="group relative bg-[#ff6f00] hover:bg-[#ff6f00]/90 text-white rounded-md overflow-hidden cursor-pointer px-8 pr-16 py-6 font-semibold text-base shadow-lg shadow-orange-500/20">{t("cta.button")}</Button></Link></div></section>
      </main>
      <Footer />
