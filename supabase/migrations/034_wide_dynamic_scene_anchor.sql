-- 034: establish dynamic scenes from their widest deliverable frame.
--
-- A close portrait cannot preserve a wall, court boundary or floor edge that
-- it never showed. Prompt system v3 renders the three-quarter frame first, then
-- gives that image to the close, medium and expression frames. Older snapshotted
-- prompt versions keep their original close-anchor topology.
...
begin
  ...
  select p_order_id, shoot.id, shoot.id::text, frame.ordinality::int,
         ...
         (frame.value->>'framing') = case
           when v_prompt_version = 'dating-scene-v3' then 'threeQuarter'
           else 'close'
         end,
