-- 036: retain the three-quarter anchor for prompt-system v5.
--
-- Migration 035 is already deployed and remains immutable. Version v5 adds a
-- portrait-first composition policy with a rare 9:16 anchor option; the render
-- topology itself remains anchor-first on the three-quarter frame.
...
begin
  ...
  select p_order_id, shoot.id, shoot.id::text, frame.ordinality::int,
         ...
         (frame.value->>'framing') = case
           when v_prompt_version in (
             'dating-scene-v3', 'dating-scene-v4', 'dating-scene-v5'
           ) then 'threeQuarter'
           else 'close'
         end,
