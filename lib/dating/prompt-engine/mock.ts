...
export function mockProductionModelCall(brief: DatingSceneBrief): PromptLabModelCall {
  return async () => {
    ...
    const wardrobeState = "The overshirt sleeves stay rolled twice at mid-forearm, every fastening and hem stays fixed, and all fabric edges are clean, continuous and intact.";
    ...
    const frames = [
      {
        ...
        width: 1728,
        height: 2304,
        prompt: `${shared} His shoulders turn twenty degrees while one hand settles the edge of his own collar and the other arm falls beyond the crop. His eyes are on the lens with a calm half-smile. A 3:4 shoulders-up opener photographed at eye level on ${technical}`,
      },
      {
        framing: "medium",
        width: 1728,
        height: 2304,
        prompt: `${shared} ${SCENE_ANCHOR_PROMPT_SENTENCE} He takes one measured step during ${brief.activity}, with the nearer hand brushing his own cuff and the far arm swinging naturally. His gaze settles toward open space beside the camera. A 3:4 chest-up candid photographed slightly below eye level on ${technical}`,
      },
      {
        framing: "threeQuarter",
        width: 1728,
        height: 2304,
        prompt: `${shared} ${SCENE_ANCHOR_PROMPT_SENTENCE} He pauses with his weight carried through one straight leg, the other knee relaxed, one hand loose near his thigh and the far hand resting at his own hip. He looks directly into the lens with quiet confidence. A 3:4 three-quarter portrait photographed at waist height on ${technical}`,
      },
      {
        framing: "expression",
        width: 1728,
        height: 2304,
        prompt: `${shared} ${SCENE_ANCHOR_PROMPT_SENTENCE} A small remembered detail from ${brief.activityReason} brings out a spontaneous laugh as his fingers touch his own sternum and the other hand hangs loosely. His gaze drops below the camera. A 3:4 close expression frame photographed just above eye level on ${technical}`,
      },
    ];
    return {
      text: JSON.stringify({
