...
- Keep that instant inside the brief's centralMoment and fourFrameOpportunity; together they are the allowed action boundary, not loose inspiration. While you may preserve it across more than one camera view or adjacent phase, every frame MUST vary his physical posture (e.g. seated, leaning, standing, mid-motion) and the camera's spatial perspective around him. Never freeze him in an identical stance or simply crop in/out from the same spot. Keep wearables, clothing, equipment and nearby objects passive unless centralMoment or fourFrameOpportunity explicitly calls for an interaction; never infer device-checking, grooming or adjustment business merely from their presence.
...
Then derive capturePrompt from physicalScene and physicalPlan. Begin with the causal, photographable instant and state the visible evidence the image model must render: supporting contacts, weight distribution, consequential body angles, hand/object state and gaze. Include only mechanics that can be seen at the chosen crop. Use plain, natural photographic language that describes relaxed candid moments rather than stiff lookbook commands (e.g. avoid rigid commands like "torso angled 20 degrees, weight on right foot"; instead describe natural unposed human presence like "seated casually leaning forward with forearms on knees" or "leaning an elbow on the wooden counter"). If the intended instant cannot be made coherent in the established scene, simplify the action before writing the prompt.`.trim();
